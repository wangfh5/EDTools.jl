...
ϕ = FBbasis(Ns, Np, :fermion, false, Nflr=Nflr)
...
ladderU3 = uuLadder(3, f=2)
ladderD2 = ddLadder(2, f=1)
...
# Apply ladders
result = apply_Ladders!(vt, v, S, ϕ.ε, nflr=Nflr)
...
Matrix(SopU3) == Matrix(creation(3, flr=2; ϕ=ϕ))
Matrix(SopD2) == Matrix(annihilation(2, flr=1; ϕ=ϕ))
Matrix(Sop23) == Matrix(hopping(3,2;flr=(2,1), ϕ=ϕ))
Matrix(pairing(3,2; flr=(2,1), ϕ=ϕ)) == creation(3;flr=2, ϕ=ϕ) * creation(2;flr=1, ϕ=ϕ)
Matrix(densities(3; flr=2, ϕ=ϕ)) == creation(3; flr=2, ϕ=ϕ) * annihilation(3; flr=2, ϕ=ϕ)
Matrix(densities(3,2; flr=(2,1), ϕ=ϕ)) == creation(3;flr=2, ϕ=ϕ) * annihilation(3;flr=2, ϕ=ϕ) * creation(2;flr=1, ϕ=ϕ) * annihilation(2;flr=1, ϕ=ϕ)
Matrix(hubbard(3;ϕ=ϕ)) == creation(3;flr=1, ϕ=ϕ) * annihilation(3;flr=1, ϕ=ϕ) * creation(3;flr=2, ϕ=ϕ) * annihilation(3;flr=2, ϕ=ϕ)